...
import { PostWithProfile, PostInsert } from '@/types/database';
...
const FixedPostCard = memo(({ post, deletePost, isDeleting }: { post: PostWithProfile, deletePost: (postId: string) => void, isDeleting: boolean }) => {
  const [showComments, setShowComments] = useState(false);
  const { user } = useAuth();
  const { isLiked, likeCount, handleLike, isLoading: isLiking } = usePostLikes({
    postId: post.id,
        ...
        <div className="flex items-center justify-between pt-2 border-t">
          <div className="flex items-center space-x-4">
            <Button
              ...
              disabled={isLiking}
              className={cn(
                "h-8 px-2 transition-all duration-200 hover:scale-105 select-none",
                isLiked
...
const FastPostCreator = memo(({ onSubmit, isLoading }: { onSubmit: (postData: PostInsert) => void; isLoading: boolean }) => {
...
export default function FixedFastHomeFeed() {
  const { posts, isLoading, error, createPost, isCreating, deletePost, isDeleting } = useSimplePosts();
      ...
      <div className="space-y-4">
        {isLoading && posts.length === 0 ? (
          ...
          posts.filter(post => post && post.id).map((post, index) => (
            ...
            >
              <FixedPostCard post={post} deletePost={deletePost} isDeleting={isDeleting} />
            </motion.div>
          ))
        )}
